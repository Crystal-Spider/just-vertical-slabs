...
public class RecipeUpdateEventHandler {
  ...
  public void onRecipesUpdatedEvent(RecipesUpdatedEvent event) {
    ...
    for(BlockState referredSlabState : VerticalSlabUtils.slabStateMap.values()) {
      creativeSearchTree.add(VerticalSlabUtils.getVerticalSlabItem(referredSlabState, VerticalSlabUtils.translucentMap.containsKey(referredSlabState.getBlock().asItem())));
    }
